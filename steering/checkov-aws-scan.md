...
- [ ] `remediation-plan.md` — parse results.json → classify ALL findings by priority (P0→P3), group by subcategory, include remediation guidance. If severity=UNKNOWN → classify per `references/severity-classification.md` (Pre-computed table → Matrix → Fallback). **HARD CONSTRAINT: mỗi finding PHẢI có row riêng với resource + file + line. KHÔNG ĐƯỢC chỉ ghi summary "8 findings" mà không liệt kê từng resource. Parse `results.failed_checks[]` → extract từng item.**
