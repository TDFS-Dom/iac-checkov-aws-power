...
- [ ] `summary.md` — parse results.json → fill TẤT CẢ fields (counts, severity, by-folder, top findings). **SEVERITY: tra `references/aws-checks-full-list.md` — KHÔNG tự classify.**
- [ ] `remediation-plan.md` — parse results.json → classify ALL findings by priority (P0→P3), group by subcategory, include remediation guidance. **SEVERITY LOOKUP (BẮT BUỘC):** Cho mỗi finding, đọc cột Severity trong `references/aws-checks-full-list.md` theo Check ID. KHÔNG tự classify. Nếu Check ID không có trong file (check mới) → dùng `references/severity-classification.md` Scoring Matrix. **HARD CONSTRAINT: mỗi finding PHẢI có row riêng với resource + file + line. KHÔNG ĐƯỢC chỉ ghi summary "8 findings" mà không liệt kê từng resource. Parse `results.failed_checks[]` → extract từng item.**
